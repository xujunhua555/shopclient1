...
public class CreatDemandActivity extends BaseActivity implements ImageItemClickListner{
	// 标题
...
//		shopMessage.setText("睁开眼睛，看着自己的双腿，它们陪你走了一生的路，可是，你们什么时候注意过它们的存在？");
	...
	private void initSelectedGridView() {
		...
		grid_instructor.setGridAdapter(selectedImageAdapter, CreatDemandActivity.this);
	...
	public void imageItemClick(int position, SelectImageItem imageItem) {
		if (imageItem != null) {
			...
			if (sid == 100) {
				...
				if (CreatDemandActivity.this.getIntent().getIntExtra("photo_message", 0) == 1) {
					goCameraActivity();
				} else {
					// 添加图片
					Intent intentImages = new Intent(CreatDemandActivity.this, SelectImagesActivity.class);
					intentImages.putExtra("image_count", imageItems.size());
					intentImages.putExtra("max_count", "1");
					startActivityForResult(intentImages, 1);
				}
			} else {
				Intent intentPicture = new Intent(CreatDemandActivity.this, ShowCreatePicturesActivity.class);
	...
	protected void callBackSuccess(ResponseEntry entry, int actionId) {
		...
		switch (actionId) {
			case Consts.actionId.resultCode:
				...
				ToastUtil.ShowToast(CreatDemandActivity.this, entry.getRepMsg());
				new Handler().postDelayed(new Runnable() {
					//
					@Override
					public void run() {
	...
	protected void callBackAllFailure(String strMsg, int actionId) {
		...
		mdialog.dismiss();
		ToastUtil.show(CreatDemandActivity.this, strMsg);
		...
		protected void onPostExecute(String result) {
			...
			if (UploadUtils.SUCCESS.equalsIgnoreCase(result)) {
				if (imageFlagNumber < mListImage.size()) {
					UploadFileTask uploadFileTask = new UploadFileTask(CreatDemandActivity.this);
				...
				ToastUtil.show(CreatDemandActivity.this, " 图片上传失败");
			}
		}
