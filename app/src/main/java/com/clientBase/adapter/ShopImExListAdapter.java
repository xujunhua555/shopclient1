...
public class ShopImExListAdapter extends BaseAdapter {
	private LayoutInflater inflater;
	private List<ExchangeModel> list_result;
	private int posIndex;
	private Context mContext;
	public ShopImExListAdapter(Context context, List<ExchangeModel> list_result) {
		mContext = context;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			holder.mTvTitle.setText(list_result.get(position).getMyShop().getShopName());
			holder.mTvMoney.setText("价格：" + list_result.get(position).getMyShop().getShopMoney() + "元");
			...
			holder.mtvTime.setText("发布时间：" + list_result.get(position).getMyShop().getShopCreatime());

			if (!TextUtils.isEmpty(list_result.get(position).getMyShop().getShopImg())) {
				Picasso.with(mContext).load(Consts.URL_IMAGE + list_result.get(position).getMyShop().getShopImg())
